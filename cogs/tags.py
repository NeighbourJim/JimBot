    ...
    async def tag(self, ctx):
        try:
            ...
            if len(tag) > 0:
                ...
                await ctx.reply(f'No tag with that name.', delete_after=6)
        except Exception as ex:
        ...
        try:
            ...
            if len(split_message) > 1 and len(split_message) < 400:
                ...
                if len(existing) == 0:
                    ...
                    ctx.command.reset_cooldown(ctx)
                    await ctx.reply(f'That tag already exists.', delete_after=5)
            else:
                await ctx.reply(f'Tag too long or too short.')
        except Exception as ex:
        ...
        try:
            ...
            if len(existing) > 0:
                ...
                if user_is_admin or ctx.message.author.id == existing[0][2]:
                    ...
                    ctx.command.reset_cooldown(ctx)
                    await ctx.reply(f'You do not have permissions to delete that tag.\nYou must either be an Administrator or the original tag creator.', delete_after=5)
            else:
                ctx.command.reset_cooldown(ctx)
                await ctx.reply(f'That tag does not exist.', delete_after=5)
        except Exception as ex:
    ...
    async def taglist(self, ctx):
        try:
            ...
            if len(tags) > 0:
                ...
                ctx.command.reset_cooldown(ctx)
                await ctx.reply(f'No tags in the database.', delete_after=5)
        except Exception as ex:
