...
class Food(commands.Cog):
    ...
    async def randomrecipe(self, ctx):
        ...
        api_results = Helpers.GetWebPage(self, url).json()
        if api_results:
            ...
            await ctx.send(f'{ctx.message.author.mention}: API Error.\nCould not contact Spoonacular API. Daily Quota possibly exceeded.')
    ...
    async def recipesearch(self, ctx):
        ...
        if split_message[0].lower() in keywords and len(split_message) > 1:
            user_kw = split_message[0].lower()
            if user_kw == "ingredient":
                user_kw = "includeIngredients"
            query = split_message[1]
        else:
            user_kw = "query"
            query = Helpers.CommandStrip(self, ctx.message.content)
        url = f'https://api.spoonacular.com/recipes/complexSearch?{user_kw}={query}&number=3&offset={random.randint(0,5)}&addRecipeInformation=true&apiKey={current_settings["keys"]["spoonacular_key"]}'
        api_results = Helpers.GetWebPage(self, url).json()
        if api_results:
